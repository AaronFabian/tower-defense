...
class GameOverState extends BaseState {
	readonly window = window as any;

	onExitGameOver: Function;

	constructor(onGameOver: Function = () => {}, onExitGameOver: Function = () => {}) {
		super();

		onGameOver();
		this.onExitGameOver = onExitGameOver;

		(this.window.gSounds as Map<string, any>).get('game-over').play();
	}
	...
	override exit() {
		(this.window.gSounds as Map<string, any>).get('game-over').stop();
